...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    epoch_step = 0
    try:
        for i, (images, gts, depths,_) in enumerate(train_loader, start=1):
            optimizer.zero_grad()
            images = images.cuda()
            gts = gts.cuda()
            depths = depths.cuda()
            ...
            if i % 100 == 0 or i == total_step or i == 1:
                print(
                    '{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], loss: {:.4f}, loss_final: {:.4f}, loss_d: {:.4f}'.
...
if __name__ == '__main__':
    ...
    # load data
    print('load data...')
    train_loader = get_loader(image_root, gt_root, depth_root, edge_root, batchsize=opt.batchsize,
